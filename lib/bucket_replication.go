...
var specChineseReplication = SpecText{
	synopsisText: "bucket的replication配置管理",
...
var specEnglishreplication = SpecText{
	synopsisText: "Replication configuration management of bucket",
...
func (replicationc *ReplicationCommand) RunCommand() error {
	...
	if strMethod == "get" {
		if strItem != "" {
			if strItem != "location" && strItem != "progress" {
