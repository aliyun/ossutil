...
func (cmd *Command) ossClient(bucket string) (*oss.Client, error) {
	...
	client, err := oss.New(endpoint, accessKeyID, accessKeySecret, oss.UseCname(isCname), oss.SecurityToken(stsToken), oss.UserAgent(getUserAgent()), oss.Timeout(120, 1200))
	if err != nil {
		return nil, err
	}
	return client, nil
