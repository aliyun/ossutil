...
var OptionMap = map[string]Option{
    ...
    OptionMultipart:        Option{"-m", "--multipart", "", OptionTypeFlagTrue, "", "", "指定操作的对象为bucket中未completed的multipart所对应uploadId，而非默认情况下的object。", "Indicate that the subject of the command is uploadIds of those multipart which has not been completed, instead of objects(which is the subject in default situation."},
    OptionAllType:          Option{"-a", "--all-type", "", OptionTypeFlagTrue, "", "", "指定操作的对象为bucket中的object和未completed的multipart所对应uploadID。", "Indicate that the subject of the command contains both objects and uploadIds of those multipart which has not been completed."},
    ...
    OptionSnapshotPath:     Option{"", "--snapshot-path", "", OptionTypeString, "", "", 
        "该选项用于在某些场景下加速增量上传批量文件（目前，下载和拷贝不支持该选项）。在cp上传文件时使用该选项，ossutil在指定的目录下生成文件记录文件上传的快照信息，在下一次指定该选项上传时，ossutil会读取指定目录下的快照信息进行增量上传。用户指定的snapshot目录必须为本地文件系统上的可写目录，若该目录不存在，ossutil会创建该文件用于记录快照信息，如果该目录已存在，ossutil会读取里面的快照信息，根据快照信息进行增量上传（只上传上次未成功上传的文件和本地进行过修改的文件），并更新快照信息。注意：因为该选项通过在本地记录成功上传的文件的本地lastModifiedTime，从而在下次上传时通过比较lastModifiedTime来决定是否跳过相同文件的上传，所以在使用该选项时，请确保两次上传期间没有其他用户更改了oss上的对应object。当不满足该场景时，如果想要增量上传批量文件，请使用--update选项。另外，ossutil不会主动删除snapshot-path下的快照信息，为了避免快照信息过多，当您确定快照信息无用时，请您自行清理snapshot-path。", 
