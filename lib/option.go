...
var OptionMap = map[string]Option{
	...
	OptionIncludeRegexp: Option{"", "--include-regexp", DefaultNonePattern, OptionTypeString, "", "",
		fmt.Sprintf("包含对象匹配模式，正则表达式，可包含文件夹，如：\\directory\\"),
		fmt.Sprintf("Exclude Directory of key, support regex, support include path, e.g., \\directory\\")},
	...
	OptionExcludeRegexp: Option{"", "--exclude-regexp", DefaultNonePattern, OptionTypeString, "", "",
		fmt.Sprintf("不包含对象匹配模式，正则表达式，可排除文件夹，如：\\directory\\"),
		fmt.Sprintf("Exclude Directory of key, support regex, support exclude path, e.g., \\directory\\")},
