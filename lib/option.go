...
var OptionMap = map[string]Option{
	...
	OptionStorageClass: Option{"", "--storage-class", DefaultStorageClass, OptionTypeString, "", "",
		fmt.Sprintf("设置对象的存储方式，默认值：%s。", DefaultStorageClass),
		fmt.Sprintf("set the storage class of bucket(default: %s).", DefaultStorageClass)},
